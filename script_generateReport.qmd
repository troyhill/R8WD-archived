...
tribeData2 <- getWQP(organization = Tribal_org,
...
## Water Quality Data Analysis Report for the `r allSummary$data$OrganizationFormalName[1]` CWA106 Program

## Introduction

Map of sampling locations, list of parameters etc.

This report focuses on a set of core physical and chemical measures common among CWA106 monitoring programs. Relevant parameters measured by `r allSummary$data$OrganizationFormalName[1]` include `r included_params`.
...
blank_table <- allSummary$blank_summary[, c(10, 3, 5:7, 9, 11)]
...
```{r}
#| echo: false
#| fig-cap: Figure 1. Field blank performance for each analyte over time. Red lines indicate detection limits for each analyte during each sampling event.
...
ggplot(allSummary$blank_proc[grepl(x = tolower(allSummary$blank_proc$ActivityTypeCode), pattern = 'field blank'), ], aes(x = date, y = ResultMeasureValue)) +
  ...
  geom_line(aes(y = DetectionQuantitationLimitMeasure.MeasureValue, x = date), col = 'red', linetype = 3) +
...
if(percent_field_blank_detects <= 10) {
  blank_issues <- 'This is enocuraging and indicates consistent attention to detail during sample collection and handling.'
} else {
...
rec_list$blank_issues     <- paste0('**Blank performance:** ', percent_field_blank_detects, '% of blanks had evidence of contamination. ' , blank_issues)
```
...
rec_list$rep_issues     <- paste0('**Replicate performance:** ', reps_pct_above20, '% of replicates had problematic levels of imprecision. ' , rep_issues)
