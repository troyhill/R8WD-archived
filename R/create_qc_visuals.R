...
create_qc_visuals <- function(dataset,
  ...
  if (grepl(x = tolower(form), pattern = 'coarse', fixed = TRUE)) {
    ...
    blk_data <- plyr::ddply(dataset$blank_proc, c('CharacteristicName'), plyr::summarize,
                            `Field blanks collected` = sum(!is.na(ResultMeasureValue)))
    rep_data <- plyr::ddply(dataset$rep_summary, c('CharacteristicName'), plyr::summarize,
                            `Field replicates collected` = sum(n))
    table1 <- plyr::join_all(list(sample_data, blk_data, rep_data), by = 'CharacteristicName')
  ...
  if (grepl(x = tolower(form), pattern = 'blank', fixed = TRUE)) {
    ...
    if (nrow(dataset$blank_proc) == 0) {
      ...
      blank_int <- plyr::ddply(dataset$blank_proc, c('ActivityTypeCode', 'CharacteristicName'), plyr::summarize,
    ...
    if (field_only) {
      table1 <- table1[grep(x = table1$`Sample Type`, pattern = 'Field'), ]
    }

    ### blank performance plot
    dataset$blank_proc$date <- as.POSIXct(dataset$blank_proc$ActivityStartDate, format = '%Y-%m-%d')
    blank_perf  <- dataset$blank_proc[grepl(x = tolower(dataset$blank_proc$ActivityTypeCode), pattern = 'field blank'), ]
    ...
    blankDat                 <- dataset$blank_proc
    blankDat$year            <- as.numeric(substr(blankDat$ActivityStartDate, 1, 4))
    blankDat$month           <- as.numeric(substr(blankDat$ActivityStartDate, 1, 4)) + (as.numeric(substr(blankDat$ActivityStartDate, 6, 7)) / 12)
    blankDat$day             <- as.numeric(substr(blankDat$ActivityStartDate, 1, 4)) + (as.numeric(substr(blankDat$ActivityStartDate, 6, 7)) / 12) + (as.numeric(substr(blankDat$ActivityStartDate, 9, 10)) / 31)

    param_yrs_wo_blank <- plyr::ddply(blankDat[, c('CharacteristicName', 'ResultMeasureValue', summaryPeriod)],
                                      c('CharacteristicName', summaryPeriod), plyr::summarize,
    ...
    blanks_expected  <- plyr::ddply(tmpDat[, c('CharacteristicName', 'ResultMeasureValue', summaryPeriod)],
                                   c('CharacteristicName', summaryPeriod), plyr::summarize,
    ...
    plot2 <- ggplot2::ggplot(data = blanks_expected[!(blanks_expected$CharacteristicName %in% parameters_wo_blanks) & !is.na(blanks_expected$ratio), ],
                             ggplot2::aes(x = get(summaryPeriod), y = ratio)) +
  ...
  if (grepl(x = tolower(form), pattern = 'rep', fixed = TRUE)) {
    dataset$rep_proc$above_target <- ifelse(dataset$rep_proc$RPD > replicate_variation_threshold, 1, 0)

    above20_by_param <- plyr::ddply(dataset$rep_proc, c('CharacteristicName', 'ActivityTypeCode'), plyr::summarize,
    ...
    if (any(!is.na(dataset$rep_proc$RPD))) {
    ...
    dataset$rep_proc$date <- as.POSIXct(sapply(X = strsplit(dataset$rep_proc$id, '__'), FUN = '[[', 6), format = '%Y-%m-%d')
    field_rep_plot_data <- dataset$rep_proc[grepl(x = dataset$rep_proc$ActivityTypeCode, pattern = 'Field replicate'), ]
    ...
    # rep_plot1 <- ggplot(dataset$rep_proc[grepl(x = dataset$rep_proc$ActivityTypeCode, pattern = 'Field replicate'), ], aes(x = date, y = RPD)) +
    ...
    ### colleciton rate plot
    ### which of these year-parameter combos have reps collected?
    repDat                 <- dataset$rep_proc
    repDat$year            <- as.numeric(substr(repDat$ActivityStartDate, 1, 4))
    repDat$month           <- as.numeric(substr(repDat$ActivityStartDate, 1, 4)) + (as.numeric(substr(repDat$ActivityStartDate, 6, 7)) / 12)
    repDat$day             <- as.numeric(substr(repDat$ActivityStartDate, 1, 4)) + (as.numeric(substr(repDat$ActivityStartDate, 6, 7)) / 12) + (as.numeric(substr(repDat$ActivityStartDate, 9, 10)) / 31)

    param_yrs_wo_reps      <- plyr::ddply(repDat[, c('CharacteristicName', 'aver', summaryPeriod)],
                                     c('CharacteristicName', summaryPeriod), plyr::summarize,
    ...
    tmpDat$month  <- as.numeric(substr(tmpDat$ActivityStartDate, 1, 4)) + (as.numeric(substr(tmpDat$ActivityStartDate, 6, 7)) / 12)
    tmpDat$day    <- as.numeric(substr(tmpDat$ActivityStartDate, 1, 4)) + (as.numeric(substr(tmpDat$ActivityStartDate, 6, 7)) / 12) + (as.numeric(substr(tmpDat$ActivityStartDate, 9, 10)) / 31)
    reps_expected <- plyr::ddply(tmpDat[, c('CharacteristicName', 'ResultMeasureValue', summaryPeriod)],
                                 c('CharacteristicName', summaryPeriod), plyr::summarize,
                                 number_of_obs       = sum(!is.na(ResultMeasureValue)))
