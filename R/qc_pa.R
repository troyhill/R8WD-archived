...
qc_pa <- function(dat,
  ...
  difference_abs <- apply(X = dat[, column_names], MARGIN = 1, FUN = function(x) {diff(range(x, na.rm = TRUE))})
  ...
  dat[, paste0(column_name, 'qc')]   <- rowMeans(dat[, column_names], na.rm = TRUE)
  ...
  graphics::plot(dat[, column_names], main = paste0('Sensor agreement check for\nsensor ', head(dat$sensor_index, 1), ', ', gsub(x = column_name, pattern = '_', replacement = ' ')), pch = 19, las = 1, cex = 0.75,
  ...
  graphics::points(dat[which_rejected, column_names], bg = 'red', pch = 21)
  # graphics::text(x = )
