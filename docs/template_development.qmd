...
blank_table             <- blank_int[, c(10, 11, 3:5, 7, 9)]
blank_table$`Blank frequency >10%?` <- ifelse(blank_freq >= 10.0, "\u2705", "\u274C")
blank_table$`<5% of blanks contaminated?` <- ifelse(blank_table$`Blanks above MDL (%)` <= 5.0, "\u2705", "\u274C")
}
```
...
if (nrow(allSummary$blank_summary) == 0)  {
  ...
  percent_field_blank_detects <- round(sum(allSummary$blank_summary$exceedances[grep(x = allSummary$blank_summary$ActivityTypeCode, pattern = 'Field Blank')], na.rm = TRUE) / sum(allSummary$blank_summary$Blanks[grep(x = allSummary$blank_summary$ActivityTypeCode, pattern = 'Field Blank')], na.rm = TRUE), 3)*100
  }
...
above20_by_param$`Rep frequency >10%?`       <- ifelse(above20_by_param$rep_freq >= 10.0, "\u2705", "\u274C")
above20_by_param$`<10% of reps high-variation?` <- ifelse(above20_by_param$`Proportion above 20%` <= 10.0, "\u2705", "\u274C")
