...
if (nrow(allSummary$rep_summary) > 1) {
rep_object <- create_qc_visuals(dataset = allSummary, 
  ...
  rep_object <- list(table = data.frame(Analyte = unique(reps_expected$CharacteristicName), 
                                        n = 0), 
                     collection_rate_plot = ggplot() + ggplot2::ggtitle('No replicates reported in WQP'), 
                     performance_plot = ggplot() + ggplot2::ggtitle('No replicates reported in WQP'))
  names(rep_object$table)[2] <- 'Number of replicate sets'
