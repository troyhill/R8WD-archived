...
years_without_blanks <- sort(years_with_data[!(years_with_data %in% years_with_blanks)])
...
above20_by_param <- plyr::ddply(allSummary$rep_proc, c('CharacteristicName'), plyr::summarize,
      `No. replicate sets`       = sum(!is.na(RPD)),
      `No. replicates above 20%` = sum(RPD > 20, na.rm = TRUE),
...
years_with_data    <- as.numeric(na.omit(unique(substr(allSummary$data$ActivityStartDateTime, 1, 4))))
years_without_reps <- sort(years_with_data[!(years_with_data %in% years_with_reps)])
