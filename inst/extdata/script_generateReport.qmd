...
tribeData1 <- dataRetrieval::readWQPdata(organization = Tribal_org,
...
allSummary <- summarizeQC(data = tribeData2) # lot of single-observation 'reps' - look into this
...
included_params <- paste0(unique(tribeData2$CharacteristicName)[1:(length(unique(tribeData2$CharacteristicName))-1)], collapse = ', ')
included_params <- sub(",([^,]*)$",", and\\1", included_params)
```

## Water Quality Data Analysis Report for `r tribeData1$OrganizationFormalName[1]`

## Introduction

Map of sampling locations, list of parameters etc.

This report focuses on a set of core physical and chemical measures common among CWA106 monitoring programs. Relevant parameters measured by `r tribeData1$OrganizationFormalName[1]` include `r included_params`.
...
allSummary$blank_summary$`Sample Type` <- sapply(strsplit(allSummary$blank_summary$ActivityTypeCode, '-'), FUN = '[[', 2)
allSummary$blank_summary$`Exceedances (%)` <- paste0(round(allSummary$blank_summary$exceedances / allSummary$blank_summary$n, 3)*100, '%')
allSummary$blank_summary[, 5] <- round(allSummary$blank_summary[, 5], 3)
names(allSummary$blank_summary)[c(3, 5:7, 9)] <- c('Analyte', 'Year', 'Blanks', 'Mean value', 'Exceedances (>MDL)')
gt(allSummary$blank_summary[, c(10, 3, 5:7, 9, 11)])
```
...
if ((length(unique(allSummary$data[allSummary$data$CharacteristicName == 'TN', 'ResultAnalyticalMethod.MethodIdentifier'])) == 1) & (unique(allSummary$data[allSummary$data$CharacteristicName == 'TN', 'ResultAnalyticalMethod.MethodIdentifier']) == 'Calculated')) {
  sonde_parameters <- c(sonde_parameters, 'TN')
}


number_of_lab_blanks <- ifelse(grepl(x = names(tmp), pattern = 'Lab'), sum(tmp[grepl(x = names(tmp), pattern = 'Lab')]), 0)
number_of_field_blanks <- ifelse(grepl(x = names(tmp), pattern = 'Field'), sum(tmp[grepl(x = names(tmp), pattern = 'Field')]), 0)
...
number_field_blank_detects <- sum(allSummary$blank_summary[, 9][grep(x = allSummary$blank_summary$ActivityTypeCode, pattern = 'Field Blank')], na.rm = TRUE)

percent_field_blank_detects <- round(sum(allSummary$blank_summary[, 9][grep(x = allSummary$blank_summary$ActivityTypeCode, pattern = 'Field Blank')], na.rm = TRUE) / sum(allSummary$blank_summary$Blanks[grep(x = allSummary$blank_summary$ActivityTypeCode, pattern = 'Field Blank')], na.rm = TRUE), 3)*100

### identify parameters without QC data (exclude sonde measurements)
parameters_without_blanks <- unique(allSummary$data$CharacteristicName)[!(unique(allSummary$data$CharacteristicName) %in% c(unique(allSummary$blank_summary$Analyte), sonde_parameters))]
...
rep_totals_by_param$label_text <- as.character(rep_totals_by_param$Freq)
rep_totals_by_param$label_text[1] <- paste0('n = ', rep_totals_by_param$Freq[1])

label_location <- as.numeric(quantile(x = allSummary$rep_proc$RPD, 0.95))
...
above20_by_param <- plyr::ddply(allSummary$rep_proc, c('CharacteristicName'), plyr::summarize,
      `No. replicate sets`       = length(RPD),
      `No. replicates above 20%` = sum(RPD > 20),
      `Proportion above 20%`     = paste0(round(100*(sum(RPD > 20) / length(RPD))), '%')
      )
...
total_reps_above20      <- sum(allSummary$rep_proc$RPD > 20)
...
variation_measure_message <- ifelse(allSummary$rep_summary$variation_measure[1] == "Coefficient of Variation", 
                                    'Some replicates sets included more than two individual replicates. This could indicate duplicate entries in WQP or other issues, or they may truly reflect exceptional levels of replication. This report could not resolve those uncertainties, and so used the coefficent of variation (CV) as the measure of variation among replicates. Lower CVs indicate less variation among replicates.', 
...
Over the period of record used for this report, `r tribeData1$OrganizationFormalName[1]` collected `r total_reps` sets of replicates. This combined total includes `r number_of_field_reps` field replicates and `r number_of_lab_reps` lab replicates. Field replicates comprised `r round(number_of_field_reps/ samples_for_rep_comparison*100)`% of all samples.

In comparison to the 20% target level of variation, replicate performance was `r ifelse(reps_pct_above20 <= 10, 'excellent', ifelse(reps_pct_above20 <= 25, 'generally good', ifelse(reps_pct_above20 <= 50, 'generally mixed', ifelse(reps_pct_above20 > 50, 'not good'))))`. `r summary_reps_above20`
...
#| warning: false
#| label: Table2
#| fig-cap: Table 2. The proportion of replicates exceeding this report's 20% target for variation.
gt(above20_by_param)
```

Replicate variation is expected to be elevated for analytes that are not homogenous throughout the water column. This group includes *E. coli*, turbidity/total suspended solids (TSS), total Kjeldahl nitrogen (TKN), total phosphorus, and other analytes that rely on particulate fractions. This variation itself may be inconsistent over time (Figure 3), varying as a function of discharge and local in-stream deposition/erosion dynamics.
...
#| echo: false
#| warning: false
#| fig-cap: Figure 3. Field replicate performance over time for each parameter. Note that y-axis scales vary across facets. A dashed horizontal line is shown in each facet at the 20% level for reference.
...
if (number_of_lab_blanks == 0) {
  lab_blank_rec <- '**Laboratory blanks** were not present in data found in the Water Quality Portal. These data appear to have been either not collected or not input into WQP. More comprehensive reporting of laboratory blanks into WQP would enable more meaningful assessment of data quality differences among datasets and laboratories.'
...
} else if (number_of_field_blanks < no_years) {
  field_blank_rec <- '**Field blanks** appear in the reported data at a rate of less than one field blank per year, indicating either minimal collection or incomplete reporting of these QC data to WQP. Increasing the frequency of field blanks to at least 10% is highly recommended.'
} else if (number_of_field_blanks < 0.10*no_samples) {
  field_blank_rec <- paste0('**Field blanks** appear in the data reported to WQP at a rate of ', round(number_of_field_blanks / no_samples*100), '% of all relevant samples (excluding sonde data). Recommendations include ensuring that field blanks are reported comprehensively and increasing the frequency of field blanks to at least 10% of samples.')
}else if (number_of_field_blanks >= 0.10*no_samples) {
...
### Quality Control Data: Replicates
...
} else if (number_of_lab_reps < no_years) {
  lab_rep_rec <- '**Laboratory blanks** appear in the reported data at a rate of less than one lab blank per year, indicating either minimal collection or incomplete reporting of these QC data to WQP. More comprehensive reporting of laboratory blanks into WQP would enable more meaningful assessment of data quality differences among datasets and laboratories.'
} else if (number_of_lab_reps < 0.10*samples_for_rep_comparison) {
  lab_rep_rec <- paste0('**Laboratory blanks** appear in the data reported to WQP, but represent just ', round(number_of_lab_reps / samples_for_rep_comparison*100), '% of all samples. Recommendations include ensuring that laboratory blanks are reported comprehensively and increasing the frequency of laboratory blanks to at least 10% of lab samples.')
} else if (number_of_lab_reps >= 0.10*samples_for_rep_comparison) {
  lab_rep_rec <- '**Laboratory blanks** represent at least 10% of all lab samples. Project Managers are commended for their high level of effort and reporting on this quality metric.'
}
...
} else if (number_of_field_reps < no_years) {
  field_rep_rec <- '**Field replicates** appear in the reported data at a rate of less than one field blank per year, indicating either minimal collection or incomplete reporting of these QC data to WQP. Increasing the frequency of field replicates to at least 10% is highly recommended.'
} else if (number_of_field_reps < 0.10*samples_for_rep_comparison) {
  field_rep_rec <- paste0('**Field replicates** appear in the data reported to WQP at a rate of ', round(number_of_field_reps / samples_for_rep_comparison*100), '% of all samples. Recommendations include ensuring that field blanks are reported comprehensively and increasing the frequency of field replicates to at least 10% of samples.')
}else if (number_of_field_reps >= 0.10*samples_for_rep_comparison) {
...
if(reps_pct_above20 <= 25) {
 rep_issues <- '**Replicate perfoemance:** Less than 25%  of replicates had undesirable levels of variation. This is very encouraging, and suggests consistency and attention to detail during sample collection and handling.'
} else {
  rep_issues <- paste0('**Replicate perfoemance:** ', reps_pct_above20, '% of replicates had undesirable levels of variation. This could indicate a need for improved consistency and attention to detail during sample collection and handling.')
}

rec_list$rep_freq_time  <- rep_collection_consistency_time
rec_list$rep_freq_param <- rep_collection_consistency_param
rec_list$rep_issues     <- rep_issues
```
